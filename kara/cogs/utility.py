...
class Utility(Cog):
    ...
    async def role(self, ctx: Context, role: Role):
        ...
        embed.add_field(name=t("role.users"), value=f"[{len(role.members)}]({post(members.encode('utf-8'))})", inline=True)
    ...
    async def compare(self, ctx: Context, role_a: Role, role_b: Role):
        ...
        embed = Embed(title=t("compare.title"), description=t("compare.description", a=role_a.mention,
                                                              b=role_b.mention),
                      timestamp=ctx.message.created_at)
